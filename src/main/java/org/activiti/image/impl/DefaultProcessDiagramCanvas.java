...
public class DefaultProcessDiagramCanvas {
   ...
   * 节点及流高亮颜色
   */
  protected static Color HIGHLIGHT_COLOR = new Color(139, 195, 74);
   ...
   * <p>
   * Allows to specify minimal boundaries on the left and upper side of the canvas. This is useful
   * for diagrams that have white space there (eg Signavio). Everything beneath these minimum values
   * will be cropped.
   *
  ...
  private static Point getLinesIntersection(Line2D a,
      Line2D b) {
    double d =
        (a.getX1() - a.getX2()) * (b.getY2() - b.getY1()) - (a.getY1() - a.getY2()) * (b.getX2() - b
            .getX1());
    double da =
        (a.getX1() - b.getX1()) * (b.getY2() - b.getY1()) - (a.getY1() - b.getY1()) * (b.getX2() - b
            .getX1());
    double ta = da / d;
    Point p = new Point();
  ...
  public void drawConnection(int[] xPoints,
    ...
    if (associationDirection.equals(AssociationDirection.ONE) || associationDirection
        .equals(AssociationDirection.BOTH)) {
  ...
  public void drawCatchingEvent(String id,
    ...
    if (icon != null) {
      ...
      int imageX = (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 2) - (icon.getWidth()
          / 2));
      int imageY = (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 2) - (icon.getHeight()
          / 2));
  ...
  public void drawLabel(String text,
      GraphicInfo graphicInfo,
      GraphicInfo LineCenterGraphicInfo,
      boolean centered) {
    float interline = 1.0f;
    ...
    if (text != null && text.length() > 0) {
      ...
      int wrapWidth = 100;
//      int textY =  "0.0".equals(graphicInfo.getY() + "") ? (int)LineCenterGraphicInfo.getY() : (int)graphicInfo.getY();
      int textY = (int)LineCenterGraphicInfo.getY();
      AttributedString as = new AttributedString(text);
      as.addAttribute(TextAttribute.FOREGROUND,
          g.getPaint());
      as.addAttribute(TextAttribute.FONT,
      ...
      while (lbm.getPosition() < text.length()) {
        ...
        textY += tl.getAscent();
        Rectangle2D bb = tl.getBounds();
//        double tX = "0.0".equals(graphicInfo.getX() + "") ? LineCenterGraphicInfo.getX() : graphicInfo.getX();
        double tX = LineCenterGraphicInfo.getX();
        if (centered) {
          tX += (int) (graphicInfo.getWidth() / 2 - bb.getWidth() / 2);
        }
        tl.draw(g,
  ...
  public void drawLabelByLineCenter(String text,
      GraphicInfo graphicInfo,
      boolean centered) {
    float interline = 1.0f;
  ...
  public void drawStartEvent(String id,
    ...
    if (icon != null) {
      int imageX = (int) Math
          .round(graphicInfo.getX() + (graphicInfo.getWidth() / 2) - (icon.getWidth() / 2));
      int imageY = (int) Math
          .round(graphicInfo.getY() + (graphicInfo.getHeight() / 2) - (icon.getHeight() / 2));
  ...
  protected void drawMultilineText(String text,
    ...
    while (measurer.getPosition() < characterIterator.getEndIndex() && currentHeight <= boxHeight) {
      ...
      int height = ((Float) (layout.getDescent() + layout.getAscent() + layout.getLeading()))
          .intValue();
    ...
    for (TextLayout textLayout : layouts) {
      ...
      currentX =
          x + (centered ? ((boxWidth - ((Double) textLayout.getBounds().getWidth()).intValue()) / 2)
              : 0);
   ...
   * <p>
   * Throws an {@link ActivitiImageException} when {@link #close()} is already called.
   */
